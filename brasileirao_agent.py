"""
=============================================================
  AGENTE BRASILEIRAO — KICK MONITOR v3
  Dashboard en vivo + Google Sheets persistente
=============================================================
"""
...
def _get_gsheets_client():
    ...
    try:
        ...
        if SHEETS_CREDS_JSON:
            ...
            creds_file = os.path.join(BASE_DIR, "gcp_credentials.json")
            if not os.path.exists(creds_file):
                return None
            creds = _SACredentials.from_service_account_file(creds_file, scopes=scopes)
...
def update_sheets(resumen: dict):
    ...
    try:
        ...
        if not ws.get_all_values():
            headers = ["Fecha","Canal","Streamer","Pais","Partido","Hora Inicio","Hora Fin",
                       "Duracion (min)","Peak Viewers","Avg Viewers","Snapshots","Min. Vistos Est."]
            ws.append_row(headers)
        row = [resumen.get("fecha",""), resumen.get("canal",""), resumen.get("nombre",""),
               resumen.get("pais",""), resumen.get("titulo",""), resumen.get("hora_inicio",""),
               resumen.get("hora_fin",""), resumen.get("duracion_min",0),
               resumen.get("peak_viewers",0), resumen.get("avg_viewers",0),
               resumen.get("total_snapshots",0), resumen.get("minutos_vistos_est",0)]
        ws.append_row(row)
        print(f"[Sheets] ✅ KPIs guardados: {resumen.get('nombre')}")
    except Exception as e:
        print(f"[Sheets] Error KPIs: {e}")

def upload_csv_to_sheets(canal: str, nombre: str):
    gc = _get_gsheets_client()
    if not gc:
        return
    ...
    try:
        ...
        if rows_to_upload:
            ...
            print(f"[Sheets] ✅ Raw data: {len(rows_to_upload)} snapshots de {nombre}")
    except Exception as e:
        print(f"[Sheets] Error raw data: {e}")
...
INTERVALO     = 60
CSV_FILE      = os.path.join(BASE_DIR, "kick_brasileirao_data.csv")
PARTIDOS_FILE = os.path.join(BASE_DIR, "kick_partidos_resumen.json")
...
state = {
    ...
    "conteo_snapshots": {}, "sesion_inicio": {}, "partidos_resumen": [], "log": [],
}
_prev_bra: dict = {}
...
.sc{background:var(--bg2);border:1px solid var(--b);border-radius:10px;padding:16px 20px;position:relative;overflow:hidden}
...
.cc.live{border-color:rgba(0,194,68,.3)}
...
.b-bra{background:rgba(0,255,102,.15);color:#fff;border:1px solid var(--g2)}
.cview{font-family:var(--ff);font-size:40px;color:#fff;line-height:1;margin-bottom:4px}
...
.dot{width:7px;height:7px;border-radius:50%;background:var(--g2);display:inline-block;margin-right:5px;animation:bk 1.2s infinite}
@keyframes bk{0%,100%{opacity:1}50%{opacity:.2}}
.sheetsbar{background:var(--bg3);border:1px solid var(--b);border-radius:8px;padding:10px 18px;display:flex;align-items:center;justify-content:space-between;margin-bottom:20px;font-family:var(--fm);font-size:11px;color:var(--muted)}
.sheetsbar strong{color:var(--g)}
.bgrid{display:grid;grid-template-columns:1fr 1fr;gap:18px;margin-bottom:20px}
...
.kv.peak{color:var(--g2)}.kv.avg{color:var(--y)}.kv.dur{color:#7eb8ff}
...
<main>
  <div class="stats">
    ...
    <div class="sc"><div class="sl">Canales en Vivo</div><div class="sv" id="s-live">0</div><div class="ss">de <span id="s-tc">10</span> monitoreados</div></div>
    <div class="sc"><div class="sl">Peak Máximo (sesión)</div><div class="sv" id="s-peak">0</div><div class="ss" id="s-peak-c">—</div></div>
    <div class="sc"><div class="sl">Partidos Registrados</div><div class="sv" id="s-partidos">0</div><div class="ss">guardados en Google Sheets</div></div>
  </div>
  <div class="sheetsbar">
  ...
  <div class="bgrid">
    <div class="panel">
      ...
      <div id="plist"><div class="empty">Cuando termine un partido, los KPIs<br>quedan guardados permanentemente en Google Sheets</div></div>
    </div>
    <div class="panel">
  ...
  <div class="panel">
    <div class="stitle">📋 Historial Completo</div>
    <div style="overflow-x:auto">
    <table id="htable" style="width:100%;border-collapse:collapse;font-family:var(--fm);font-size:12px">
...
async function refresh(){
  ...
  document.getElementById('s-total').textContent=fmt(live.reduce((s,c)=>s+(c.viewers_actuales||0),0));
  document.getElementById('s-live').textContent=live.length;
  document.getElementById('s-tc').textContent=d.canales.length;
  document.getElementById('s-partidos').textContent=(d.partidos_resumen||[]).length;
  let maxP=0,maxC='—';
  ...
  if(!d.partidos_resumen||!d.partidos_resumen.length){
    pl.innerHTML='<div class="empty">Cuando termine un partido, los KPIs<br>quedan guardados permanentemente en Google Sheets</div>';
  } else {
  ...
  if(d.partidos_resumen&&d.partidos_resumen.length){
    tbody.innerHTML=d.partidos_resumen.map((p,i)=>`<tr style="background:${i%2===0?'transparent':'rgba(255,255,255,.03)'}">
      ...
      <td style="padding:7px 12px;max-width:300px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${p.titulo||'—'}</td>
...
if __name__ == "__main__":
    print("""
╔══════════════════════════════════════════════════════╗
║   🏆  BRASILEIRAO EN KICK — MONITOR v3              ║
║   Dashboard en vivo + Google Sheets persistente      ║
╠══════════════════════════════════════════════════════╣
